...
{
	CHUNK_GEN("Mass Chunk Gen", "The server is generating a lot of chunks, causing the server to lag, and even spike.", "Create a world border, or pregenerate your world within a border to optimize chunk loading. Take a look at the plugin World Border."),
	...
	TNT_EXPLOSIONS("TNT Lag", "The server is consuming a lot of memory for explosions from tnt. The server may also be lagging from tnt also.", "To fix this, you may want to change some of the react configuration to crack down on massive amounts of tnt exploading at once."),
	CHUNKS("Chunk Lag", "The server has too many chunks loaded for the server memory to handle. This can be caused by too many players, or a plugin preventing a relative massive amount of chunks from unloading.", "Consider upgrading your memory, however if this is not possible, try unloading worlds, or removing any plugins that prevent a massive amount of chunk unloads."),

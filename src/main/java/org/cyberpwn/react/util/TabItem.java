...
public class TabItem
{
	...
	protected Object toPacket(final Tab tab, final int action)
	{
		if(profile == null)
		{
			try
			{
				profile = NMSClass.GameProfile.getConstructor(UUID.class, String.class).newInstance(uuid, name);
				
				if(skin != null)
				{
					Bukkit.getScheduler().runTaskAsynchronously(React.instance(), new Runnable()
					{
						@Override
						public void run()
						{

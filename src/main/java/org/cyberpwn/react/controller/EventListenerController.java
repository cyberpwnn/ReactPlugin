...
public class EventListenerController extends Controller
{
	...
	public void addTraces(Event e)
	{
		for(RegisteredListener i : e.getHandlers().getRegisteredListeners())
		{
			...
			if(!events.containsKey(i.getPlugin()))
			{
				events.put(i.getPlugin(), new GList<String>());
			}
			
			if(!events.get(i.getPlugin()).contains(e.getClass().getSimpleName()))
			{
				events.get(i.getPlugin()).add(e.getClass().getSimpleName());
			}
		}
	}

...
import org.cyberpwn.react.updater.utils.TOTP;
...
public class SpigotUserManager implements UserManager
{
	...
	public User getUserById(int userid, User user)
	{
		try
		{
			...
			if(doc.select("dl.lastActivity").size() != 0)
			{
				...
				{
					reqUser.setLastActivity(doc.select("dl.lastActivity").get(0).select("dd").get(0).text());
				}
			}
			return reqUser;
		}
	...
	@Override
	public User authenticate(String username, String password) throws InvalidCredentialsException, TwoFactorAuthenticationException, GeneralSecurityException
	{
	...
	@Override
	public User authenticate(String username, String password, String totpSecret) throws InvalidCredentialsException, TwoFactorAuthenticationException, GeneralSecurityException
	{
		SpigotUser user = new SpigotUser(username);
	...
	@Override
	public User authenticate(String username, String password, User user) throws InvalidCredentialsException, TwoFactorAuthenticationException, GeneralSecurityException
	{
		...
		HTTPResponse res = Request.post(url, spigotUser.getCookies().size() != 0 ? spigotUser.getCookies() : SpigotSiteCore.getBaseCookies(), params);
		
		if(res.getHtml().contains("Incorrect password. Please try again."))
		{
			UpdateController.invalid = true;
		...
		if(res.getResponseURL().toString().startsWith("https://www.spigotmc.org/login/two-step"))
		{
			...
			if(providerInput == null || !providerInput.val().equalsIgnoreCase("totp"))
			{
				...
				totpResponse = Request.get("https://www.spigotmc.org/login/two-step?remember=1&provider=totp", spigotUser.getCookies(), new HashMap<String, String>());
			}
		...
		else
		{
			...
			spigotUser.setUserId(Integer.parseInt(StringUtils.getStringBetween(res.getHtml(), "member\\?user_id=(.*?)\">")));
			spigotUser.setToken(doc.select("input[name=_xfToken]").get(0).attr("value"));
	...
	private static SpigotUser totpVerification(SpigotUser user) throws GeneralSecurityException
	{
		...
		try
		{
			...
			params.put("redirect", SpigotSiteCore.getBaseURL());
			HTTPResponse res = Request.post(url, user.getCookies(), params);
			Document doc = res.getDocument();
			user.setCookies(res.getCookies());
			// Fetch data
			user.setUsername(doc.select("a.username.NoOverlay").first().text());
			user.setUserId(Integer.parseInt(StringUtils.getStringBetween(res.getHtml(), "member\\?user_id=(.*?)\">")));
			user.setToken(doc.select("input[name=_xfToken]").get(0).attr("value"));
			return user;
		}
	...
	public void logOff(User user, boolean force)
	{
		...
		else
		{
			try
			{
				HTTPResponse res = Request.get(SpigotSiteCore.getBaseURL() + "/logout?_xfToken=" + ((SpigotUser) user).getToken(), ((SpigotUser) user).getCookies(), new HashMap<String, String>());
				SpigotUser spigotUser = (SpigotUser) user;
				spigotUser.setCookies(res.getCookies());
				return;
			}
	...
	public boolean isLoggedIn(User user)
	{
		try
		{
			HTTPResponse res = Request.get(SpigotSiteCore.getBaseURL(), ((SpigotUser) user).getCookies(), new HashMap<String, String>());
			res = handleTwoStep(res, (SpigotUser) user);
			Document doc = res.getDocument();
	...
	public boolean untrustThisDevice(User user)
	{
		try
		{
			...
			Request.post(SpigotSiteCore.getBaseURL() + "account/two-step/trusted-disable", ((SpigotUser) user).getCookies(), params);
	...
	public List<String> getUsernamesByName(String name)
	{
		...
		try
		{
			...
			HTTPResponse res = Request.post(url, SpigotSiteCore.getBaseCookies(), params);
	 ...
	 *            HTTP response that could possible be a 2FA
	 * @param user
	 *            User to use
	 * @return success
	 */
	public static HTTPResponse handleTwoStep(HTTPResponse originalResponse, SpigotUser user) throws TwoFactorAuthenticationException
	{
		...
		if(originalResponse.getResponseURL().toString().startsWith("https://www.spigotmc.org/login/two-step"))
		{
			...
			{
				// Redirect to TOTP Two step (not email or whatever ,...)
				res = Request.get("https://www.spigotmc.org/login/two-step?remember=1&provider=totp", user.getCookies(), new HashMap<String, String>());
			}
